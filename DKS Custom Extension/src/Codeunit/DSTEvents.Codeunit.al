codeunit 50000 "DST Events"
{
    ...
    var CU51004: Codeunit "DKS Check Tvungen Dimension";
    ...
    begin
        ...
        if Rec."No." <> '' then if Rec."VAT Prod. Posting Group" <> '' then Rec.Validate(Rec."VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
            else
            begin
                if Rec."VAT Bus. Posting Group" <> '' then Rec.Validate(Rec."VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
                else
...
*/
    ...
    begin
        ...
        if not DSTSetup.Get()then exit;
        if not DSTSetup."Use Department as VAT Bus. pos" then exit;
        if Rec."Account Type" = Rec."account type"::"G/L Account" then if Rec."Account No." <> '' then if Rec."VAT Prod. Posting Group" <> '' then Rec.Validate(Rec."VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
                else
                begin
                    if Rec."VAT Bus. Posting Group" <> '' then Rec.Validate(Rec."VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
                    else
        ...
        if Rec."Bal. Account Type" = Rec."bal. account type"::"G/L Account" then if Rec."Bal. Account No." <> '' then if Rec."Bal. VAT Prod. Posting Group" <> '' then Rec.Validate(Rec."Bal. VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
                else
                begin
                    if Rec."Bal. VAT Bus. Posting Group" <> '' then Rec.Validate(Rec."Bal. VAT Bus. Posting Group", Rec."Shortcut Dimension 1 Code")
                    else
    ...
    begin
        //>> COR/MIN 190324
        if Rec."Document Type" in[Rec."Document Type"::Invoice, Rec."Document Type"::"Credit Memo"]then if(xRec."Posting Date" <> 0D)then if(Rec."Posting Date" <> 0D)then if(xRec."Posting Date") <> (Rec."Posting Date")then if(xRec."Due Date" <> 0D)then if(Rec."Due Date" <> xRec."Due Date")then begin
    ...
    begin
        ...
        if Rec."Shortcut Dimension 1 Code" <> xRec."Shortcut Dimension 1 Code" then begin
            ...
            if PurchaseLine_Loc.FindSet()then repeat PurchaseLine_Loc.Validate("VAT Bus. Posting Group", PurchaseLine_Loc."Shortcut Dimension 1 Code");
